...
export class WebSocketService {
  ...
  connect(): Promise<boolean> {
      ...
      this.checkServerAvailability(GameConfig.apiUrl)
        .then((isAvailable) => {
          if (!isAvailable) {
            this.status = "error";
            throw new Error(
              "Server is not responding to HTTP request. Is it running?",
            );
          }
          ...
          try {
            ...
            this.socket.on("disconnect", (reason) =>
              this.handleSocketClose(reject, reason),
            );
            this.socket.on("connect_error", (error) =>
              this.handleSocketError(reject, error),
            );
            ...
            this.socket.on("pong", (data) =>
              this.handleMessage({ type: "pong", data }),
            );
            this.socket.on("game_state", (data) =>
              this.handleMessage({ type: "game_state", data }),
            );
            this.socket.on("move", (data) =>
              this.handleMessage({ type: "move", data }),
            );
            this.socket.on("action", (data) =>
              this.handleMessage({ type: "action", data }),
            );
            this.socket.on("room_info", (data) =>
              this.handleMessage({ type: "room_info", data }),
            );
            this.socket.on("test_response", (data) =>
              this.handleMessage({ type: "test_response", data }),
            );
    ...
    this.connectionPromise = this.connectionPromise.catch((error) => {
      ...
      if (this.reconnectAttempts < this.maxReconnectAttempts) {
        ...
        this.logDebug(
          `Connection failed. Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`,
        );
  ...
  sendMessage(type: string, data: unknown): Promise<void> {
    ...
    if (!this.socket || !this.isConnected) {
      this.logDebug(
        `Not connected when trying to send ${type}. Attempting to connect...`,
      );
      ...
      return this.connect().then((connected) => {
        ...
        if (!this.socket || !this.isConnected) {
          this.logError(
            "Cannot send message - still not connected after connection attempt",
          );
  ...
  private setupPingPong(): void {
    ...
    this.pingInterval = window.setInterval(() => {
      ...
      // Send ping
      this.sendMessage("ping", { timestamp: now }).catch((error) => {
  ...
  async testConnection(): Promise<{
    ...
    try {
      ...
      result.serverReachable = await this.checkServerAvailability(
        GameConfig.apiUrl,
      );
      ...
      if (!this.isConnected) {
        ...
        try {
          ...
          result.webSocketState = this.socket
            ? this.socket.connected
              ? "CONNECTED"
              : "DISCONNECTED"
            : null;
        ...
        try {
          ...
          result.error =
            error instanceof Error
              ? error.message
              : "Unknown error during ping test";
        }
      }
    } catch (error) {
      result.error =
